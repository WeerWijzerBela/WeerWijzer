...
def get_metingen(locatie: str, db: Session = Depends(get_db)) -> List[WeerMetingUren]:
    ...
    try:
        ...
        if not locatie_db:
            raise HTTPException(
                status_code=404,
                detail=f"Locatie {locatie} bestaat niet in de database.",
            )

        # Haal de meest recente meting op
        recentste_meting = (
            db.query(DBMeting)
            .filter(DBMeting.locatieId == locatie_db.locatieId)
            .order_by(DBMeting.datetime.desc())
            .first()
        )
        if not recentste_meting:
            raise HTTPException(
                status_code=404,
                detail=f"Geen metingen gevonden voor locatie {locatie}.",
            )
...
def create_meting(
    ...
    try:
        locatie_db = (
            db.query(DBLocatie)
            .filter(DBLocatie.locatie == nieuwe_meting.locatie)
            .first()
        )
        if not locatie_db:
...
def delete_metingen(
    ...
    try:
        ...
        if len(metingen) <= 1:
            return {
                "detail": f"Er is slechts één / geen meting voor locatie {locatie}. Deze wordt niet verwijderd."
            }
        ...
        metingen_to_delete = (
            ...
            .join(DBLocatie)
            .filter(DBLocatie.locatie == locatie)
            .filter(DBMeting.datetime < cutoff_time)
            .all()
        )

        # Verwijder de metingen ouder dan 12 uur
        for meting in metingen_to_delete:
            db.query(DBMetingUren).filter(
                DBMetingUren.metingenId == meting.metingenId
...
def get_voorspellingen(locatie: str, db: Session = Depends(get_db)) -> List[VoorspellingUren]:
    ...
    try:
        ...
        voorspellinguren = (
            db.query(DBVoorspellingUren, DBzWaarden.beschrijving)
            .join(
                DBVoorspelling,
                DBVoorspelling.voorspellingenId == DBVoorspellingUren.voorspellingenId,
            )
        ...
        if not voorspellinguren:
            raise HTTPException(
                status_code=404,
                detail=f"Geen voorspellingen gevonden voor locatie {locatie}.",
            )
...
def create_voorspelling(
    ...
    try:
        locatie_db = (
            db.query(DBLocatie)
            .filter(DBLocatie.locatie == nieuwe_voorspelling.locatie)
            .first()
        )
        if not locatie_db:
            raise HTTPException(status_code=404, detail="Locatie niet gevonden")

        voorspelling_db = DBVoorspelling(
...
def create_voorspelling_uren_batch(
    ...
    try:
        ...
        max_voorspellingen_id = db.query(
            func.max(DBVoorspelling.voorspellingenId)
        ).scalar()
        ...
        db.rollback()
        logging.error(
            f"[API] Er is een fout opgetreden bij het toevoegen van nieuwe voorspellinguren: {e}"
        )
        raise HTTPException(status_code=500, detail="Interne serverfout")
...
def delete_voorspellingen(
    ...
    try:
        ...
        meest_recente_voorspelling = (
            db.query(DBVoorspelling)
            .join(DBLocatie)
            .filter(DBLocatie.locatie == locatie)
            .order_by(DBVoorspelling.voorspellingenId.desc())
            .first()
        )

        # Verwijder alle voorspellingen behalve de meest recente
        db.query(DBVoorspellingUren).filter(
            DBVoorspellingUren.voorspellingenId.in_(
                db.query(DBVoorspelling.voorspellingenId)
                .join(DBLocatie)
                .filter(
                    DBLocatie.locatie == locatie,
                    DBVoorspelling.voorspellingenId
                    != meest_recente_voorspelling.voorspellingenId,
                )
            )
        ).delete(synchronize_session=False)

        db.query(DBVoorspelling).filter(
            DBVoorspelling.locatieId == locatie_db.locatieId,
            DBVoorspelling.voorspellingenId
            != meest_recente_voorspelling.voorspellingenId,
        ).delete(synchronize_session=False)
...
#####Test endpoint voor testen van pipelines
@app.get('/test')
def test():
    return {"detail": "Test geslaagd!"}